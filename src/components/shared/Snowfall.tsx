...
/**
 * Canvas snowfall: density, randomized speed/size, wind drift, opacity,
 * direction and colour. Skips the animation loop when reduced motion is preferred.
 */
export function Snowfall(props: SnowfallProps) {
  ...
  useEffect(() => {
    ...
    function draw() {
      const { color: flakeColor } = cfg.current;
      g.clearRect(0, 0, W, H);
      g.fillStyle = flakeColor;
    ...
    function loop(t: number) {
      ...
      for (let i = 0; i < flakes.length; i++) {
        ...
        f.y += f.vy * dirSign;
        f.x +=
          wBase + f.vx * wVar + Math.sin(t * 0.0012 + f.phase) * f.sway;
